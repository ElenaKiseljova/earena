  ...
  document.addEventListener('DOMContentLoaded', () => {
    try {
      ...
      window.form = (attr) => {
        ...
        // Если задано, то содержимое этого элемента будет очищаться при отправке формы и заменяться содержимым шаблона
        let SELECTOR_FOR_TEMPLATE_REPLACE = attr.selectorForTemplateReplace ? attr.selectorForTemplateReplace : '';
        ...
        if (FORM_CHECK) {
          ...
          /******* START functions ********/
              ...
              FORM_CHECK.addEventListener('submit', (evt) => {
                ...
                */
                ...
                var onSuccess = (response) => {
                  ...
                  if ( ID_FORM.indexOf('contact') > -1 ) {
                    let openPopupButtonsSuccessContactForm = FORM_CHECK.querySelector('button[name="success"].openpopup');
                    let openPopupButtonsErrorContactForm = FORM_CHECK.querySelector('button[name="error"].openpopup');
            ...
            let additionButtonClosePopup = () => {
              let formClosePopups = document.querySelectorAll('.button--popup-close');
